...
class SignupForm(forms.Form):
    ...
    email = forms.EmailField(help_text="Required. A valid email address for verification.")
...
class PaginatedListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["search_placeholder"] = getattr(
            self, "search_placeholder", self.model.__name__.lower() + "s"
        )
        return context
...
class TopMatchesView(PaginatedListView):
    ...
    def get_queryset(self):
        ...
        qs = qs.annotate(
            sort_score=Coalesce(
                ...
                ExpressionWrapper(F("cagematch_rating") * 10.0, output_field=FloatField()),
    ...
    def get_context_data(self, **kwargs):
        ...
        context["ratings_available"] = Match.objects.filter(cagematch_rating__isnull=False).exists()
        # Promotion list for the filter dropdown.
        context["filter_promotions"] = (
            Promotion.objects.filter(events__matches__isnull=False).distinct().order_by("name")[:30]
        )
        return context
...
def signup(request):
    ...
    if not allowed:
        ...
        return render(request, "signup.html", {"form": SignupForm(), "page_title": "Sign Up"})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            EmailVerificationToken.objects.create(
                user=user, token=token, expires_at=timezone.now() + timedelta(hours=expiry_hours)
            )
    ...
    try:
        ...
        if not verification.is_valid:
            if verification.used:
                ...
                messages.error(
                    request, "This verification link has expired. Please request a new one."
                )
        ...
        messages.success(
            request, "Email verified! You can now contribute to OWDB. Welcome to the community!"
        )
...
def verification_pending(request):
    ...
    return render(request, "verification_pending.html", {"page_title": "Verify Your Email"})
...
def resend_verification(request):
    ...
    expiry_hours = getattr(settings, "EMAIL_VERIFICATION_TOKEN_EXPIRY_HOURS", 24)
    EmailVerificationToken.objects.create(
        user=request.user, token=token, expires_at=timezone.now() + timedelta(hours=expiry_hours)
    )
...
def login_view(request):
    ...
    if not allowed:
        ...
        return render(request, "login.html", {"form": AuthenticationForm(), "page_title": "Login"})
...
class Hot100View(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["available_rankings"] = (
            Hot100Ranking.objects.filter(is_published=True)
            .values("year", "month")
