...
class WrestleBotActivityAdmin(admin.ModelAdmin):
    list_display = [
        'created_at', 'action_badge', 'entity_type', 'entity_link',
        'source', 'status_badge', 'duration_display', 'ai_badge'
    ]
    list_filter = ['action_type', 'entity_type', 'source', 'success', 'ai_assisted', 'created_at']
    search_fields = ['entity_name', 'source', 'error_message']
    readonly_fields = [
        'created_at', 'action_type', 'entity_type', 'entity_id', 'entity_name',
...
class WrestleBotStatsAdmin(admin.ModelAdmin):
    list_display = [
        'date', 'discoveries_display', 'enrichments_display',
        'images_display', 'errors_display', 'api_calls_display'
    ]
    list_filter = ['date']
    readonly_fields = [
        'date', 'discoveries', 'enrichments', 'images_added', 'verifications',
    ...
    def discoveries_display(self, obj):
        ...
        if obj.discoveries > 0:
            return format_html('<span style="color:#28a745; font-weight:bold;">+{}</span>', obj.discoveries)
        return '0'
    ...
    def enrichments_display(self, obj):
        ...
        if obj.enrichments > 0:
            return format_html('<span style="color:#17a2b8; font-weight:bold;">{}</span>', obj.enrichments)
        return '0'
    ...
    def images_display(self, obj):
        ...
        if obj.images_added > 0:
            return format_html('<span style="color:#fd7e14; font-weight:bold;">{}</span>', obj.images_added)
        return '0'
    ...
    def errors_display(self, obj):
        ...
        if obj.errors > 0:
            return format_html('<span style="color:#dc3545; font-weight:bold;">{}</span>', obj.errors)
