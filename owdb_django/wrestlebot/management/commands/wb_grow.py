...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--dry-run", action="store_true", help="Print top candidates without fetching anything."
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"    wrestlers persisted: {stats.wrestler_persisted}")
        )
        self.stdout.write(self.style.SUCCESS(f"    events persisted   : {stats.event_persisted}"))
        self.stdout.write(self.style.SUCCESS(f"    venues persisted   : {stats.venue_persisted}"))
        if stats.promotion_persisted:
            self.stdout.write(
                self.style.SUCCESS(f"    promotions persisted: {stats.promotion_persisted}")
            )
        self.stdout.write(self.style.WARNING(f"  skipped generic       : {stats.skipped_generic}"))
        self.stdout.write(
            self.style.WARNING(f"  skipped unclassified  : {stats.skipped_unclassified}")
        )
        self.stdout.write(
            self.style.WARNING(f"  skipped no-content    : {stats.skipped_no_content}")
        )
        self.stdout.write(
            self.style.WARNING(f"  skipped extract-fail  : {stats.skipped_extract_failed}")
        )
