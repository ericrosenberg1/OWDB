...
def _evaluate_candidate(
    ...
    *,
    ...
    if meta.attribution_required and not meta.artist:
        return None, "License requires attribution but no Artist in metadata", 0

    # Promotional-art guard. For event entities, refuse files whose
    # primary subject is a copyrighted promotional design (poster,
    # keyart, press kit). The photo may be CC-licensed but the design
    # underneath isn't — and unlike a promotion logo (nominative fair
    # use), we'd be using the design as the image of the event itself,
    # which is the use that draws cease-and-desists.
    #
