...
class WrestleBotActivity(models.Model):
    ...
    source = models.CharField(
        max_length=100, help_text="Data source (e.g., wikipedia, cagematch, claude_api)"
    )
    ...
    duration_ms = models.PositiveIntegerField(
        null=True, blank=True, help_text="How long the operation took in milliseconds"
    )
...
class FieldProvenance(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.entity_type}#{self.entity_id}.{self.field_name} <- {self.source_fetch.source}"
        )
...
class EarlObservation(models.Model):
    ...
    severity = models.CharField(max_length=20, db_index=True, help_text="warning | error")
    ...
    times_seen = models.PositiveIntegerField(
        default=1, help_text="Increments each audit that sees this same issue"
    )
...
class RuleScore(models.Model):
    ...
    times_fired = models.PositiveIntegerField(
        default=0, help_text="Rule produced a non-default result"
    )
    true_positives = models.PositiveIntegerField(
        default=0, help_text="Fired and the call was correct"
    )
    false_positives = models.PositiveIntegerField(
        default=0, help_text="Fired but the call was wrong"
    )
    ...
    notes = models.TextField(
        blank=True, default="", help_text="Earl's free-text notes about this rule"
    )
...
class RuleSuggestion(models.Model):
    ...
    target_rule_id = models.CharField(
        max_length=100,
        blank=True,
        default="",
    ...
    kind = models.CharField(
        max_length=20, blank=True, default="", help_text="guard | extractor | cleanup | consistency"
    )
    ...
    sample_observations = models.JSONField(
        default=list, blank=True, help_text="EntityIDs / values that motivated the suggestion"
    )
...
class AgentSession(models.Model):
    ...
    outcome = models.CharField(
        max_length=20, choices=OUTCOME_CHOICES, default="running", db_index=True
    )
    final_summary = models.TextField(
        blank=True, default="", help_text="Agent's own summary of what it accomplished"
    )
    ...
    def cost_estimate_usd(self) -> float:
        ...
        return (self.input_tokens_used * in_rate + self.output_tokens_used * out_rate) / 1_000_000
...
class AgentToolCall(models.Model):
    ...
    result_summary = models.TextField(
        blank=True, default="", help_text="Truncated result text for human inspection"
    )
