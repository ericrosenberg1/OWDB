...
def check_match(match) -> list[ConsistencyIssue]:
    ...
    if (
        match.winning_side is not None
...
def check_image_legal_use(entity_type: str, entity) -> list[ConsistencyIssue]:
    ...
    if license_code in ("cc-by", "cc-by-sa") and credit.lower() in _NULL_ATTRIBUTION:
        issues.append(
            ConsistencyIssue(
                severity="error",
                field="image_credit",
...
def log_issues(
    entity_type: str, entity_id: int, entity_name: str, issues: list[ConsistencyIssue]
) -> int:
    """
