...
class TVEpisodeScraper:
    ...
    def scrape_show_episodes(
        ...
        tmdb_episodes = self._get_tmdb_episodes(show.tmdb_id, start_date, end_date, limit)

        logger.info("Found %d episodes from TMDB for %s", len(tmdb_episodes), show.name)
        ...
        for ep_data in tmdb_episodes:
            try:
                ...
                logger.error("Error processing episode %s: %s", ep_data.get("episode_number"), e)
                results["errors"] += 1
    ...
    def _create_or_update_episode(self, show, ep_data: Dict) -> tuple:
        """
    ...
    def poll_for_new_episodes(self) -> Dict[str, Any]:
        ...
        active_shows = TVShow.objects.filter(finale_date__isnull=True, tmdb_id__isnull=False)
    ...
    def get_missing_episodes(
        ...
        tmdb_episodes = self._get_tmdb_episodes(
            show.tmdb_id, start_date or show.premiere_date, end_date or date.today(), limit=5000
        )

        # Get existing episode numbers
        existing = set(
            Event.objects.filter(tv_show=show).values_list("episode_number", "season_number")
        )
        ...
        try:
            ...
            for i, match_data in enumerate(event_data.get("matches", [])):
                ...
                for wrestler_name in wrestler_names:
                    ...
                    wrestler = Wrestler.objects.filter(name__iexact=wrestler_name.strip()).first()
                ...
                if winner_name:
                    winner = Wrestler.objects.filter(name__iexact=winner_name.strip()).first()
                    if winner:
                        match.winner = winner
                        match.save()
