...
class AssignImageRefusalTests(TestCase):
    ...
    def test_refuses_when_too_small(self):
        ...
        self.assertTrue(
            any("too small" in (c.get("refusal_reason") or "").lower() for c in result.considered)
        )
    ...
    def test_refuses_attribution_required_without_artist(self):
        ...
        self.assertFalse(result.success)
        self.assertTrue(
            any("attribution" in (c.get("refusal_reason") or "").lower() for c in result.considered)
        )
...
class IdentityScoringTests(TestCase):
    ...
    def test_filename_full_name_matches(self):
        self.assertTrue(
            commons.filename_mentions_name(
                "Bret_Hart_in_2009.jpg",
                "Bret Hart",
            )
        )
    ...
    def test_filename_year_only_doesnt_match(self):
        ...
        self.assertFalse(
            commons.filename_mentions_name(
                "Random_unrelated_photo_2024.jpg",
                "Bret Hart",
            )
        )
    ...
    def test_description_match(self):
        self.assertTrue(
            commons.description_mentions_name(
                "Bret Hart wrestling at WrestleMania VIII",
                "Bret Hart",
            )
        )
    ...
    def test_description_no_match(self):
        self.assertFalse(
            commons.description_mentions_name(
                "Shawn Michaels delivering Sweet Chin Music",
                "Bret Hart",
            )
        )
    ...
    def test_description_with_digits_preserved(self):
        ...
        self.assertTrue(
            commons.description_mentions_name(
                "Members of the tag team 2point0 at AEW Dynamite",
                "2point0",
            )
        )
    ...
    def test_p18_yielded_first_then_category_then_body(self):
        ...
        with (
            ...
            mock.patch(
                "owdb_django.wrestlebot.pipeline.images.fetch_image_for_qid",
                return_value={
                    "filename": "Test_Wrestler_P18.jpg",
                    "url": "",
                    "thumb_url": "",
                    "qid": "Q12345",
                    "property": "P18",
...
class CascadeAcceptanceTests(TestCase):
    ...
    def _meta(
        self, *, license_code="cc-by-sa", description="", width=400, height=600, categories=None
    ):
        return commons.CommonsImageMeta(
    ...
    def test_first_fill_dim_floor_relaxed(self):
        ...
        with (
            ...
            self.assertTrue(
                any(
                    "too small" in (c.get("refusal_reason") or "").lower() for c in r_old.considered
                )
            )
    ...
    def test_identity_floor_blocks_weak_body_image(self):
        ...
        self.assertIn("identity confidence", result.considered[0]["refusal_reason"].lower())
