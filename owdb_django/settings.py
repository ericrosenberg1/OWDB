...
if not DEBUG:
    ...
    # ...except /health/. The container healthcheck reaches gunicorn over plain
    # HTTP on localhost:8000, where this redirect fires and returns a 301 before
    # the view ever runs. `curl -f` treats 3xx as success, so the check reported
    # healthy for as long as gunicorn could emit a redirect -- including the six
    # weeks the write path was dead in ROS-1204. Exempting the path makes the
    # endpoint honest for every internal caller. (ROS-1207)
...
# SQLite must create a rollback journal ('<db>-journal') *next to* the database
# file on every write transaction, so the containing directory has to be
# writable by the app user -- not just the .sqlite3 file itself. In the
# container image /app is root:root 0755 while gunicorn runs as 'appuser', so a
# DB at BASE_DIR/db.sqlite3 opens fine for reads and then fails every write
# with "attempt to write a readonly database" (older SQLite: "unable to open
# database file"). SQLITE_PATH lets the deploy point the DB at a writable,
# bind-mounted directory instead. Default keeps the repo-root path for local
# dev, where the checkout is writable. (ROS-1204)
...
# =============================================================================
# Initialised last so Django config (APP_ENV) is already loaded. The deployed
# tree had no Sentry wiring at all, which is why six weeks of production
# signup 500s never raised an alert (ROS-1204).
# Interactive and one-off sessions do not report. An operator mistyping an
# import inside `manage.py shell` used to open a production issue -- four of the
# six "new issues" in the Jul 25-Aug 1 weekly digest were that, not real faults
# (ROS-1409, ROS-1411). Denylist, not allowlist: anything unrecognised still
# reports, because quietly losing a production error is the worse failure.
...
def is_non_reporting_process(argv):
    ...
    if not argv:
        return False
    # `python -c "..."`. The bare REPL (argv[0] == '') is deliberately NOT
    # matched: embedded interpreters such as mod_wsgi also leave argv[0] empty.
    if argv[0] == '-c':
        return True
