...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--limit", type=int, default=50,
                            help="Max events to process this run (default 50).")
    ...
    def handle(self, *args, **options):
        ...
        if options["event_id"]:
            ...
            event_ids = set(
                SourceFetch.objects
                .filter(entity_type="event", source="wikipedia", http_status=200)
                .values_list("entity_id", flat=True)
                .distinct()
            )
            if not options["rerun_all"]:
                already_done = set(
                    Match.objects.values_list("event_id", flat=True).distinct()
                )
        ...
        for ev in events:
            ...
            if stats.get("error"):
                self.stdout.write(self.style.WARNING(
                    f"  Event#{ev.id} {ev.name[:50]}: {stats['error']}"
                ))
                continue
